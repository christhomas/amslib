...
class Amslib_Plugin3
{
	...
	protected function process()
	{
		...
		$source = Amslib_Router4::getObject("source:xml");
	...
	protected function installLanguages($name,$lang)
	{
		foreach($lang["language"] as $langCode){
			...
			if(isset($lang["router"])){
				Amslib_Router_Language4::add($langCode,str_replace("_","-",$langCode));
			}

			//	Now register all the languages with the application
			Amslib_Plugin_Application3::registerLanguage($name, $langCode);
		}
	}
	...
	protected function createAPI()
	{
		...
		if($api == false) $api = new Amslib_MVC6();
	...
	protected function createTranslator($name)
	{
		...
		$language = Amslib_Plugin_Application3::getLanguage($name);
		...
		foreach($this->config as $key=>$block){
			 if($key == "model"){
				...
				if(isset($block["import"]) && $block["import"] != $this->getName()){
					$plugin = Amslib_Plugin_Manager3::getPlugin($block["import"]);
					if($plugin){
						$this->setConfig($key,$plugin->getConfig($key));
						if(isset($block["move"])) $plugin->removeConfig($key);
					}else Amslib_FirePHP::output("AP3::transfer(): plugin invalid",$item);
				}else if(isset($block["export"]) && $block["export"] != $this->getName()){
					$plugin = Amslib_Plugin_Manager3::getPlugin($block["export"]);
					if($plugin){
				...
				foreach(Amslib_Array::valid($block) as $item){
					if(isset($item["import"]) && $item["import"] != $this->getName()){
						$plugin = Amslib_Plugin_Manager3::getPlugin($item["import"]);
						if($plugin){
						...
						$plugin = Amslib_Plugin_Manager3::getPlugin($item["export"]);
						if($plugin){
							$plugin->setConfig(array($key,$item["name"]),$item);
							if(isset($item["move"])) $this->removeConfig($key,$item["name"]);
						}else Amslib_FirePHP::output("AP3::transfer(): plugin invalid",array($item,$plugin));
					}
				}
	...
	public function config($name,$location)
	{
			...
			foreach($list as $node){
				switch($node->nodeName){
					...
					case "path":{
						...
						foreach($child as $c){
							$v = Amslib_Plugin3::expandPath($c->nodeValue);

							if($c->nodeName == "include"){
								Amslib::addIncludePath(Amslib_File::absolute($v));
							}else{
								Amslib_Plugin3::setPath($c->nodeName,$v);

								switch($c->nodeName){
									case "plugin":{
										Amslib_Plugin_Manager3::addLocation($v);
									}break;
					...
					default:{
						$this->config[$node->nodeName] = Amslib_Plugin3::expandPath($node->nodeValue);
					}break;
				}
			}
		}
		...
		foreach($list as $node){
			if($this->getName() != $node->nodeValue){
				$plugin = Amslib_Plugin_Manager3::config($node->nodeValue,$this->location);

				if($plugin){
					$this->config["requires"][$node->nodeValue] = $plugin;
				}else{
					Amslib_FirePHP::output("AP3::config(), child plugin config failed",array($node->nodeValue,$location));
				}
			}
		}
	...
	public function load()
	{
		if($this->isReady)
		{
			...
			Amslib_Plugin_Manager3::insert($this->name,$this);
