 ...
 *
 * file: Amslib_Plugin_Application3.php
...
class Amslib_Plugin_Application3 extends Amslib_Plugin3
{
	...
	protected function initRouter()
	{
		...
		$r = Amslib_Router4::getInstance();
		Amslib_Router4::setSource(Amslib_Router4::getObject("source:xml"));
	}
	...
	protected function executeRouter()
	{
		...
		$xml = Amslib_Router4::getObject("source:xml");
		$xml->load($source);

		Amslib_Router4::setSource($xml);
		Amslib_Router4::execute();
		
		//	hack into place the automatic adding of all the stylesheets and javascripts
		$p = Amslib_Router4::getParameter("plugin");
		if($p){
			$s = Amslib_Router4::getStylesheets();
			$j = Amslib_Router4::getJavascripts();
			$p = Amslib_Plugin_Manager3::getAPI($p);
	...
	public function __construct($name,$location)
	{
		...
		Amslib_Plugin_Manager3::preload($name,$this);

		//	Set the base locations to load plugins from
		Amslib_Plugin_Manager3::addLocation($location);
		Amslib_Plugin_Manager3::addLocation($location."/plugins");
	...
	{
		$plugin		=	Amslib_Router4::getParameter("plugin",false);
		$handler	=	Amslib_Router4::getParameter("service",false);

		if($plugin && $handler){
			$api = Amslib_Plugin_Manager3::getAPI($plugin);
	...
	{
		if(Amslib_Router4::getResource() === "Service") $this->runService();
