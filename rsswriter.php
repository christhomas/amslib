...
// $Id: rsswriter.php,v 1.1 2005/11/12 00:52:06 chris_a_thomas Exp $
//
...
function channelinfo() {
	...
	foreach($keys as $k){
		$info .= "\t\t<${k}>" . htmlspecialchars($this->chaninfo[$k]) . "</${k}>\n";
	}
	
	$image = "";
	if (isset($this->image)) {
		$image = "\t\t<image rdf:resource=\"" . htmlspecialchars($this->image["uri"]) . "\" />\n";
	}
	
	$resource = "";
	foreach ($this->items as $i) {
		$resource .= "\t\t\t\t<rdf:li rdf:resource=\"" . htmlspecialchars($i["uri"]) . "\" />\n";
	}
	
print <<< END_HTML
	<channel rdf:about="$this->channelURI">\n
		$info
		$image
		<items>\n
			<rdf:Seq>\n
