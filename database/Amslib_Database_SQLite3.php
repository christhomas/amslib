 ...
 * title: Antimatter Database: SQLite3.x Implementation
...
class Amslib_Database_SQLite3 extends Amslib_Database
{	
	/**
	...
	protected function makeConnection()
	{
		...
		try{ 
			$database = new PDO("sqlite:{$this->loginDetails["database"]}"); 
 ...
 *****************************************************************************/
	...
	public function getResults($numResults,$resultHandle=NULL)
	{
		...
		for($a=0;$a<$numResults;$a++){
			$this->lastResult[] = $resultHandle->fetch(PDO::FETCH_ASSOC);
		}
	...
	public function select($query,$numResults=0)
	{
		...
		$this->selectResult = $this->connection->query("select $query");
		if($this->debug) print("<pre>QUERY = 'select $query'<br/></pre>");
		
		if($this->selectResult){
			$rowCount = $this->selectResult->rowCount();
...
DATABASE_ERROR;
	...
	public function update($query)
	{
		...
		if($result) return $this->connection->changes() >= 0;
	...
	public function delete($query)
	{
		...
		if($result) return $this->connection->changes();
	...
	{
		if($this->connection){
			return "<pre>".print_r($this->connection->errorInfo(),true)."</pre>";
		}
